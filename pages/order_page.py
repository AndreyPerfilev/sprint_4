...
class PageOrder(BasePage):
    ...
    def send_name(self, data):
        self.send_data(OrderPageLocators.name_field, data)

    @allure.step('Заполняем поле фамилия значением {data}')
    def send_sure_name(self, data):
        self.send_data(OrderPageLocators.sure_name_field, data)

    @allure.step('Заполняем поле адрес значением {data}')
    def send_address(self, data):
        self.send_data(OrderPageLocators.address_field, data)

    @allure.step('Заполняем поле метро значением {data}')
    def send_metro_field(self, data):
        self.send_data(OrderPageLocators.metro_field, data)
        self.click_on_element(OrderPageLocators.first_element_on_list)

    @allure.step('Заполняем поле телефон значением {data}')
    def send_phone(self, data):
        self.send_data(OrderPageLocators.telephone_field, data)
    ...
    def wait_load_page(self):
        self.wait_element(OrderPageLocators.title_order_page)

    @allure.step('Получаем тест заголовка страницы оформления заказа')
    def get_title_text(self):
        return self.get_text_from_element(OrderPageLocators.title_order_page)
