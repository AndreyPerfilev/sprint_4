...
class PageRent(BasePage):
    ...
    def send_date_plus_three_days(self):
        self.send_data(self.when_income_samokat, self.calculate_plus_three_days_for_order())

    @allure.step('Выбор черного самоката')
    def click_black_samokat(self):
        self.click_on_element(self.colour_samokat_black)

    @allure.step('Заполняем комментарий для курьера {data}')
    def send_comment_for_courier(self, data):
        self.send_data(self.comment_for_courier, data)
